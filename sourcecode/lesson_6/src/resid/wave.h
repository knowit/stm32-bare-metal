...
public:
  ...
  void writeFREQ_LO(uint8_t);
  void writeFREQ_HI(uint8_t);
  void writePW_LO(uint8_t);
  void writePW_HI(uint8_t);
...
inline void WaveformGenerator::clock()
{
  ...
  uint32_t accumulator_prev = accumulator;
  ...
  if (!(accumulator_prev & 0x080000) && (accumulator & 0x080000)) {
    uint32_t bit0 = ((shift_register >> 22) ^ (shift_register >> 17)) & 0x1;
    shift_register <<= 1;
...
inline void WaveformGenerator::clock(int delta_t)
{
  // No operation if test bit is set.
  if (test) {
    return;
  ...
  // Calculate new accumulator value;
  uint32_t delta_accumulator = delta_t * freq;
...
inline uint16_t WaveformGenerator::output____()
{
  return 0x000;
}
...
inline uint16_t WaveformGenerator::output___T()
{
  uint32_t msb = (ring_mod ? accumulator ^ sync_source->accumulator : accumulator)
    & 0x800000;
...
inline uint16_t WaveformGenerator::output__S_()
{
...
inline uint16_t WaveformGenerator::output_P__()
{
...
inline uint16_t WaveformGenerator::outputN___()
{
  return
...
// The sample is output if the pulse output is on.
// 
inline uint16_t WaveformGenerator::output__ST()
{
  // WaveformDA_1 = AND_mask[(WaveformDA_triangle_1 & WaveformDA_sawtooth_1)] << 4;
  return (AND_mask[output__S_() & output___T()] << 4);
  // return wave__ST[output__S_()] << 4;
}

inline uint16_t WaveformGenerator::output_P_T()
{
  return (AND_mask[output_P__() & output___T()] << 4);
  // return (wave_P_T[output___T() >> 1] << 4) & output_P__();
}

inline uint16_t WaveformGenerator::output_PS_()
{
  return (AND_mask[output__S_() & output_P__()] << 4);
  // return (wave_PS_[output__S_()] << 4) & output_P__();
}

inline uint16_t WaveformGenerator::output_PST()
{
  return (AND_mask[output_P__() & output__S_() & output___T()] << 4);
...
//
inline uint16_t WaveformGenerator::outputN__T()
{
  return 0;
}

inline uint16_t WaveformGenerator::outputN_S_()
{
  return 0;
}

inline uint16_t WaveformGenerator::outputN_ST()
{
  return 0;
}

inline uint16_t WaveformGenerator::outputNP__()
{
  return 0;
}

inline uint16_t WaveformGenerator::outputNP_T()
{
  return 0;
}

inline uint16_t WaveformGenerator::outputNPS_()
{
  return 0;
}

inline uint16_t WaveformGenerator::outputNPST()
{
  return 0;
}
...
// ----------------------------------------------------------------------------
inline uint16_t WaveformGenerator::output()
{
